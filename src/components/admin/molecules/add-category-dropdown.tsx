...
const AddCategoryDropdown: React.FC = () => {
	...
	const [poepleClicked, setPoepleClicked] = useState(false)
	const [poepleClickedItem, setPoepleClickedItem] = useState('')
	...
	const mainRef = useDetectClickOutside({
		onTriggered: () => {
			...
			setPoepleClicked(false)
		},
	})

	const handlePeopleMenuOptionClick = (e: string) => {
		if (e === categoryInputType) {
			setIsCategoryInputOpen(!isCategoryInputOpen)
		} else {
		...
		if (e !== 'meetings') {
			if (e === poepleClickedItem) {
				setPoepleClicked(!poepleClicked)
			} else {
				setPoepleClicked(true)
			}
			setPoepleClickedItem(e)
		}
	}

	const handleMainMenuOptionsClick = (e: string) => {
		setPoepleClicked(false)
		if (e === mainMenuClickedItem) {
	...
	return (
		<>
			...
			>
						...
						{peopleDropdownMenu.map((item, index) => {
							return (
								<Box
									...
									position='relative'
									backgroundColor={
										poepleClicked && poepleClickedItem === item
											? '#FFEED1'
											: 'transparent'
									}
				...
				{isCategoryInputOpen && <AddCategoryInput category={categoryInputType} />}
			</Flex>
		</>
	)
}
